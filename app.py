    ...
    with col2:
        ...
        with tab_register:
            ...
            with st.form("register_form"):
                ...
                sys_opts = {"Select a water system": None}
                for s in systems:
...
def show_mobile_nav(current_page: str, user: dict, systems: list):
    ...
    current_idx = (
        ...
        if current_page in page_keys else 0
    )
    ...
    with col2:
        if st.button("Sign out", key="mobile_signout", use_container_width=True):
...
def show_sidebar():
    ...
    with st.sidebar:
        st.markdown(
            ...
            f"<span style='font-size:20px; font-weight:700; color:#0ea5e9'>"
            f"Maji360</span><br>"
        ...
        if len(systems) == 1:
            selected = systems[0]
            st.markdown(
                f"<span style='font-size:12px; color:#94a3b8'>System</span><br>"
