...
class WaterSystem(Base):
    ...
    customers   = relationship("Customer",
                               back_populates="system")
    readings    = relationship("DailyReading",
                               back_populates="system")
    bills       = relationship("Bill",
                               back_populates="system")
    nrw_records = relationship("NRWRecord",
                               back_populates="system")
...
class User(Base):
    ...
    created_at   = Column(DateTime,
                          default=datetime.utcnow)
    system       = relationship("WaterSystem",
...
class Asset(Base):
    ...
    readings    = relationship("MeterReading",
...
def get_engine():
    ...
    if not db_url:
        db_url = os.environ.get(
            "DATABASE_URL", "sqlite:///maji360.db"
        )
