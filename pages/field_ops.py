...
def _show_tank_level_section(system_id: int,
    ...
    tank_options = {a.name: a for a in tank_assets}
    ...
    with st.form("tank_level_form"):
        level_input = st.number_input(
            ...
            step       = 0.05,
            format     = "%.2f",
            help       = "Read directly from the "
                         "level gauge on the tank wall"
        )
...
def show():
    ...
    if uses_mwater:
        st.info(
            "ℹ️ This system uses mWater for pump "
            "and tank outlet readings. Use the Sync "
            "page to pull latest data from mWater."
        )
    ...
    with tab1:
        ...
        asset_options = {
            ...
            for a in assets
            if a.asset_type in [
                "pump_house", "tank", "other"
            ]
        }

        if not asset_options:
            st.info(
                "No pump house or tank assets found."
            )
        else:
    ...
    with tab3:
        ...
        if all_rdgs:
            monthly = defaultdict(
                lambda: {
                    "pumped": 0.0, "consumed": 0.0
                }
            )
            for r in all_rdgs:
                month = r.reading_date.strftime(
                    "%Y-%m"
                )
                if r.reading_type == "pump" \
                   and r.volume:
