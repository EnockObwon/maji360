...
def show():
    ...
    st.markdown(
        ...
        f"{system_name} · Non-Revenue Water Analysis</span>",
    ...
    fig.add_trace(go.Bar(
        name="Pumped (m³)", x=months, y=pumped,
        marker_color="#bfdbfe", opacity=0.7
    ))
    fig.add_trace(go.Bar(
        name="To consumers (m³)", x=months, y=consumed,
        marker_color="#3b82f6"
    ))
    fig.add_trace(go.Scatter(
        name="NRW %", x=months, y=nrw_pct,
    ...
    fig.add_hline(
        ...
        opacity=0.5, annotation_text="20% threshold",
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title="NRW %", overlaying="y", side="right",
                    range=[0, max(nrw_pct) * 1.3 + 5],
    ...
    for year in years:
        ...
        status = ("🔴 ALERT" if ypc >= 20 else
                  "🟡 WARN"  if ypc >= 15 else "🟢 OK")
        c1, c2, c3, c4 = st.columns(4)
        with c1: st.metric(f"{year} pumped",   f"{yp:.0f} m³")
        with c2: st.metric(f"{year} consumed", f"{yc:.0f} m³")
        with c3: st.metric(f"{year} NRW",      f"{yn:.0f} m³")
        with c4: st.metric(f"{year} NRW %",    f"{ypc}% {status}")

    st.markdown("""
    <div style='font-size:12px;color:#94a3b8;margin-top:8px'>
    ℹ️ Annual figures are most reliable as tank storage
    changes cancel out over 12 months.
    Adj NRW requires weekly tank level readings.
    </div>
    """, unsafe_allow_html=True)
