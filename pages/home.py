...
def show():
    ...
    # Cash received — payment date basis (Option A) 
    # This is now the primary "Collected" figure — matches mWater.
    ...
    collection_rate = round((cash_received / total_billed) * 100, 1) if total_billed > 0 else 0

    # Previous period for deltas 
    sorted_months = sorted(all_period_months)
    prev_billed = prev_cash = prev_rate = None
    ...
    if selected_period not in (None, "All time") and len(selected_period) == 7:
        ...
        if idx > 0:
            ...
            prev_rate   = round((prev_cash / prev_billed) * 100, 1) if prev_billed > 0 else 0
    ...
    # Daily water readings chart
    with col_right:
        st.markdown("### Daily water readings")
        st.caption("Last 10 days · 🔵 Pumped from pump house · 🟢 Consumed from tank outlet")

        if recent_readings:
            by_date = {}
            for r in sorted(recent_readings, key=lambda x: x.reading_date):
                d = r.reading_date.strftime("%d %b")
                if d not in by_date:
                    by_date[d] = {"pumped": 0, "consumed": 0}
